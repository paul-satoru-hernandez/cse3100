...
void * thread_c(void * arg_in)
{
    ...
    while (! done) {
        ...
        pthread_mutex_unlock(&arg->mutex);
        int check = gmn_check(&gmn, arg->guess);
        pthread_mutex_lock(&arg->mutex);
        arg->result = check;
        if (arg->result == 0) {
...
int main(int argc, char *argv[])
{
    ...
    pthread_t threads[2];
    if (pthread_create(&threads[0], NULL, thread_c, &arg)) die("Thread failed.");
    if (pthread_create(&threads[1], NULL, thread_p, &arg)) die("Thread failed.");
