...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int max = gmn_get_max();
    //int i = 0;
    close(fdc[0]);  // Closes read file descriptor
    close(fdp[1]);  // Closes write file descriptor
    write(fdc[1], &max, sizeof(int));
    do {
        read(fdp[0], &guess, sizeof(int));
        int check = gmn_check(&gmn, guess);
        write(fdc[1], &check, sizeof(int));
        if(check == 0){
            break;
        }
    } while (1);
