...
void * printer_main(void * arg)
{
    // TODO
    int remaining;
    printer_t * printer = arg;
    job_queue_t *pdata = printer->jq;

    while (1) {
        pthread_mutex_lock(&pdata->mutex);
        remaining = q_num_jobs(pdata);
        if (remaining <= 0) {
            pthread_mutex_unlock(&pdata->mutex);
            break;
        }
        int job = q_fetch_job(pdata, printer->id);
        pthread_mutex_unlock(&pdata->mutex);
        print_job(job);
        printer->njobs++;
    }
...
int main(int argc, char *argv[])
{
     ...
     *  */
    pthread_t threads[num_printers];
    pthread_mutex_init(&job_queue.mutex, NULL);
    for (int i = 0; i < num_printers; i++) {
        printers[i].id = i;
        printers[i].jq = &job_queue;
        printers[i].njobs = 0;
        printers[i].thread_id = threads[i];
        if (pthread_create(&threads[i], NULL, printer_main, &printers[i])) die("Thread failed.");
    }

    for (int i = 0; i < num_printers; i++) {
        if (pthread_join(threads[i], NULL)) die("Join failed.");
    }
