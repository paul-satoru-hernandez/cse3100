...
//old number is this player's number in the previous round
//your_increment is the opponent's increase in the current round
int my_increment(int old_number, int your_increment)
{
	int inc;

	if(100 - old_number <= 10)
		inc = 100 - old_number;
	else
		inc = 11 - your_increment;
	return inc;
}
	...
	do
	{
		...
		if(inc >=1 && inc <=10)
		{
			...
			else
			{
				...
				int my_inc = my_increment(cur_number, inc);
